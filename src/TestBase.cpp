...
void TestBase::ConsumeCmdLineArguments(int argc, char** argv)
{
  ...
  int option_index = 0;
  static struct option long_options[] = {{"masterplan", required_argument, NULL, 'm'},{"m", required_argument, NULL, 'm'},
                                         {"rolesetdir", required_argument, NULL, 'd'}, {"rd", required_argument, NULL, 'd'},
                                         {"roleset", required_argument, NULL, 'r'}, {"r", required_argument, NULL, 'r'},
                                         {"teamColour", required_argument, NULL, 'c'}, {"tc", required_argument, NULL, 'c'},
                                         {"delayStart", required_argument, NULL, 't'}, {"ds", required_argument, NULL, 't'},
                                         {"simulator", no_argument, NULL, 's'},{"s", no_argument, NULL, 's'},
                                         {"stepEngine", no_argument, NULL, 'e'}, {"se", no_argument, NULL, 'e'},
                                         {0, 0, NULL, 0}};
  ...
  while (1)
  {
    ...
    switch (opt)
    {
      ...
      case '?':
        ...
        break;
      default:
        printf("getopt returned character code '0%o', which is the char '%c'.\n", opt, opt);
    }
  }

  // check whether all required cnd-line params were set
  if (this->masterPlan.empty()) {
    cerr << "ERROR: The required command line parameter 'masterplan' is missing." << endl;
    PrintUsage();
  }
...
void TestBase::PrintUsage() {
	...
	cout << "\t--masterplan=<plan-name>\t Defines the ALICA top-level plan, which should be executed." << endl;
	cout << endl;
	cout << "Optional parameters (ALICA):" << endl;
	cout << "\t--rolesetdir=<rolesetdir>\t Defines the directory, in which the roleset file should be located." << endl;
	cout << "\t--roleset=<roleset>\t\t Defines the file, which contains the roleset." << endl;
	cout << "\t--delayStart=<milliseconds>\t Defines the time span between initialising everything and starting the ALICA engine." << endl;
	cout << "\t--stepEngine\t\t\t Makes the control-loop of the ALICA engine triggered by the user instead of a timer." << endl;
	cout << endl;
	cout << "Optional parameters (MSL):" << endl;
	cout << "\t--teamColour=[cyan|magenta]\t Defines the team colour of your team." << endl;
	cout << "\t--simulator\t\t\t Makes the control-loop of the ALICA engine triggered by the user instead of a timer." << endl;
}
