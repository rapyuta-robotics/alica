#include "Behaviour/AttackOpp.h"
#include <memory>

/*PROTECTED REGION ID(inccpp1402489351885) ENABLED START*/
// Add additional includes here
/*PROTECTED REGION END*/

namespace alica
{
/*PROTECTED REGION ID(staticVars1402489351885) ENABLED START*/
// initialise static variables here
/*PROTECTED REGION END*/

AttackOpp::AttackOpp(IAlicaWorldModel* wm)
        : DomainBehaviour(wm, "AttackOpp")
{
    /*PROTECTED REGION ID(con1402489351885) ENABLED START*/
    // Add additional options here
    /*PROTECTED REGION END*/
}
AttackOpp::~AttackOpp()
{
    /*PROTECTED REGION ID(dcon1402489351885) ENABLED START*/
    // Add additional options here
    /*PROTECTED REGION END*/
}
void AttackOpp::run(void* msg)
{
    /*PROTECTED REGION ID(run1402489351885) ENABLED START*/
    // Add additional options here
    /*PROTECTED REGION END*/
}
void AttackOpp::initialiseParameters()
{
    /*PROTECTED REGION ID(initialiseParameters1402489351885) ENABLED START*/
    // Add additional options here

    /*PROTECTED REGION END*/
}
/*PROTECTED REGION ID(methods1402489351885) ENABLED START*/
// Add additional methods here
/*PROTECTED REGION END*/

} /* namespace alica */
