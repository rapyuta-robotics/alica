...
void Constraint1417424512343::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan>) {
    ...
    // Proteced
    shared_ptr<autodiff::Term> x = dynamic_pointer_cast<autodiff::Variable>(c->getStaticVars()[0]);
    if (!x.operator bool()) {
        cout << "Constraint1417424512343::getConstraint !!!!!!!!!!!!!!!!!!!!! error" << endl;
    }
    c->editStaticRanges()[0].first = 0;
    c->editStaticRanges()[0].second = 10000;

    shared_ptr<autodiff::Term> y = dynamic_pointer_cast<autodiff::Variable>(c->getStaticVars()[1]);
    if (!y.operator bool()) {
        cout << "Constraint1417424512343::getConstraint !!!!!!!!!!!!!!!!!!!!! error" << endl;
    }
    c->editStaticRanges()[1].first = 0;
    c->editStaticRanges()[1].second = 10000;
