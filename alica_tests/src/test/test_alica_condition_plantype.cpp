...
TEST_F(AlicaConditionPlanType, conditionPlanTypeTest)
{
    ...
    alica::USummand* uSummandPreConditionPlan = tc->getPlan(1418042796751)->getUtilityFunction()->getUtilSummands()[0].get();
    ...
    dbrPre->robotId = tc->getLocalAgentId();

    alica::USummand* uSummandOtherPlan = tc->getPlan(1418042819203)->getUtilityFunction()->getUtilSummands()[0].get();
    ...
    dbrOther->robotId = tc->getLocalAgentId();

    alica::USummand* uSummandRunPlan = tc->getPlan(1418042806575)->getUtilityFunction()->getUtilSummands()[0].get();
    ...
    dbrRun->robotId = tc->getLocalAgentId();
    tc->startEngine();
    for (int i = 0; i < 21; i++) {
        tc->stepEngine();
