                ...
                for (const alica::Transition* t : plan->getTransitions()) {
                    ...
                    case 1402489071510:
                        checkTransition(t, 1402489071510, "TryToDefendGoalToGetGoal", "TryToDefendGoalToGetGoal", 1678986049909129132, 1402489037735,
                                1402488910751, "AlwaysFalseCondition", "", "", "DefaultPlugin", true);
                        break;
                    default:
                        EXPECT_TRUE(false);
...
protected:
    std::vector<std::string> getTestFolderPaths() const override
    {
        ...
        return std::vector<std::string>{path + "/hairy", path + "/nase", path + "/plans", path + "/roles", path + "/tasks", path + "/behaviours", path + "/conditions", path + "/configurations"};
    }
};
