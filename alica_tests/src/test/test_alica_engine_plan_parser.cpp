...
class AlicaEngineTest : public ::testing::Test
{
  ...
  protected:
    ...
    static void checkState(alica::State *s, long id, string name, string comment, initializer_list<long> absPlanIDs,
                           initializer_list<long> inTransitions, initializer_list<long> outTransitions,
                           long entryPointID = 0)
    {
...
TEST_F(AlicaEngineTest, planParser)
{
    ...
    for (auto iter : plans)
    {
        ...
        case 1402488634525:
            ...
            cout << "States: " << endl;
            EXPECT_EQ(2, iter.second->getStates().size()) << "Number of states didnt fit AttackPlan.pml state size."
                                                          << endl;
            ...
            for (alica::Transition *t : iter.second->getTransitions())
            {
                ...
                {
                case 1402489459382:
                    checkTransition(t, 1402489459382, "MISSING_NAME", "", 1402489460549, 1402488646220, 1402489396914,
                    ...
                    break;
                case 1402489460694:
                    checkTransition(t, 1402489460694, "MISSING_NAME", "", 1402489462088, 1402489396914, 1402488646220,
                                    "Condition-Name-Shoot-Attack", "", "Some nice comment!", "DefaultPlugin", true);
                    break;
                default:
                    EXPECT_TRUE(false);
                    cerr << "TEST_F(AlicaEngineTest, planParser) found a state not part of AttackPlan.pml!" << endl;
        ...
        case 1402488893641:
            ...
            cout << "States: " << endl;
            EXPECT_EQ(4, iter.second->getStates().size()) << "Number of states didnt fit Defend.pml state size."
                                                          << endl;
            for (alica::State *s : iter.second->getStates())
            {
                cout << "\t" << s->getName() << " ID: " << s->getId() << endl;
        ...
        case 1402488870347:
            ...
            checkPreCondition(iter.second->getPreCondition(), 1402489131988, "PreCondition", "Test PC", "",
                              "DefaultPlugin", true);
            cout << "States: " << endl;
            EXPECT_EQ(3, iter.second->getStates().size()) << "Number of states didnt fit GoalPlan.pml state size."
                                                          << endl;
            for (alica::State *s : iter.second->getStates())
            {
                cout << "\t" << s->getName() << " ID: " << s->getId() << endl;
            ...
            for (alica::Transition *t : iter.second->getTransitions())
            {
                ...
                {
                case 1402489173167:
                    checkTransition(t, 1402489173167, "MISSING_NAME", "", 1402489174338, 1402488881799, 1402489152217,
                                    "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                case 1402489205153:
                    checkTransition(t, 1402489205153, "MISSING_NAME", "", 1402489206278, 1402489152217, 1402488881799,
                                    "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                case 1402489216617:
                    checkTransition(t, 1402489216617, "MISSING_NAME", "", 1402489218027, 1402489152217, 1402489192198,
                                    "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                default:
                    EXPECT_TRUE(false);
        ...
        case 1402488437260:
            ...
            cout << "States: " << endl;
            EXPECT_EQ(5, iter.second->getStates().size()) << "Number of states didnt fit MasterPlan.pml state size."
                                                          << endl;
            for (alica::State *s : iter.second->getStates())
            {
                cout << "\t" << s->getName() << " ID: " << s->getId() << endl;
                ...
                {
                case 1402488437261:
                    checkState(s, 1402488437261, "Attack", "", {1402488866727}, {}, {1402488517667, 1409218318661},
                               1402488437263);
                    break;
                case 1402488463437:
            ...
            for (alica::Transition *t : iter.second->getTransitions())
            {
                ...
                {
                case 1402488557864:
                    checkTransition(t, 1402488557864, "GoalToSucGoal", "GoalToSucGoal", 1402488558741, 1402488470615,
                                    1402488536570, "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                case 1402488517667:
                    checkTransition(t, 1402488517667, "AttackToGoal", "AttackToGoal", 1402488519140, 1402488437261,
                                    1402488477650, "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                case 1402488519757:
                    checkTransition(t, 1402488519757, "MidFieldToGoal", "MidFieldToGoal", 1402488520968, 1402488477650,
                                    1402488470615, "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                case 1409218318661:
                    checkTransition(t, 1409218318661, "AttackToDefend", "AttackToDefend", 1409218319990, 1402488437261,
                                    1402488463437, "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                default:
                    cout << t->getId() << "########" << endl;
                    EXPECT_TRUE(false);
        ...
        case 1402488770050:
            ...
            for (alica::Transition *t : iter.second->getTransitions())
            {
                ...
                {
                case 1402489257607:
                    checkTransition(t, 1402489257607, "MISSING_NAME", "", 1402489258509, 1402488787818, 1402489237914,
                                    "MISSING_NAME", "", "", "DefaultPlugin", true);
                    break;
                case 1402489276995:
                    checkTransition(t, 1402489276995, "MISSING_NAME", "", 1402489278408, 1402488787818, 1402489273401,
                                    "MISSING_NAME", "", "", "DefaultPlugin", true);
                    EXPECT_EQ(1402500865502, t->getSyncTransition()->getId()) << "Wrong synctransition ID!" << endl;
                    break;
                case 1402500843072:
                    checkTransition(t, 1402500843072, "MISSING_NAME", "", 1402500844446, 1402500830885, 1402500833246,
                                    "MISSING_NAME", "", "", "DefaultPlugin", true);
        ...
        case 1402489318663:
            ...
            cout << "States: " << endl;
            EXPECT_EQ(1, iter.second->getStates().size()) << "Number of states didnt fit Tackle.pml state size."
                                                          << endl;
            for (alica::State *s : iter.second->getStates())
            {
                cout << "\t" << s->getName() << " ID: " << s->getId() << endl;
        ...
        default:
            EXPECT_TRUE(false);
            cerr << "TEST_F(AlicaEngineTest, planParser) found an id not part of testplans but expect_true for all ids "
                    "failed!"
                 << endl;
            break;
        }
