#include <alica_tests/behaviours/SuccessOnInitBeh.h>
#include <memory>

/*PROTECTED REGION ID(inccpp3821787310391665935) ENABLED START*/
// Add additional includes here
/*PROTECTED REGION END*/

namespace alica
{
/*PROTECTED REGION ID(staticVars3821787310391665935) ENABLED START*/
// initialise static variables here
/*PROTECTED REGION END*/

SuccessOnInitBeh::SuccessOnInitBeh(BehaviourContext& context)
        : DomainBehaviour(context)
{
    /*PROTECTED REGION ID(con3821787310391665935) ENABLED START*/
    // Add additional options here
    /*PROTECTED REGION END*/
}
SuccessOnInitBeh::~SuccessOnInitBeh()
{
    /*PROTECTED REGION ID(dcon3821787310391665935) ENABLED START*/
    // Add additional options here
    /*PROTECTED REGION END*/
}
void SuccessOnInitBeh::run(void* msg)
{
    /*PROTECTED REGION ID(run3821787310391665935) ENABLED START*/
    // Add additional options here
    /*PROTECTED REGION END*/
}
void SuccessOnInitBeh::initialiseParameters()
{
    /*PROTECTED REGION ID(initialiseParameters3821787310391665935) ENABLED START*/
    // Add additional options here
    setSuccess();

    /*PROTECTED REGION END*/
}
/*PROTECTED REGION ID(methods3821787310391665935) ENABLED START*/
// Add additional options here
/*PROTECTED REGION END*/

} /* namespace alica */
