...
TEST_F(TestInheritBlackboard, testInheritBlackboardFlag)
{
    ...
    EXPECT_TRUE(alica::test::Util::getBasicBehaviour(ae.get(), 831400441334251600, 0)->getInheritBlackboard());
    // Plan has inheritBlackboard set to false
    EXPECT_TRUE(alica::test::Util::getBasicPlan(ae.get(), 1692837668719979400, 0)->getInheritBlackboard());
}
