...
TEST_F(FailureHandling, continueOnFailure)
{
    ...
    ASSERT_TRUE(alica::test::Util::isStateActive(ae.get(), 1530004915641));

    SimpleSwitches::set(0, true);
    ac->stepEngine();

    ASSERT_TRUE(alica::test::Util::isStateActive(ae.get(), 1530069246104));

    SimpleSwitches::set(1, true);
    ac->stepEngine();

    ASSERT_TRUE(alica::test::Util::isStateActive(ae.get(), 1530004975275));
    ...
    ASSERT_TRUE(alica::test::Util::isStateActive(ae.get(), 1532424097662));
}
} // namespace
} // namespace alica
