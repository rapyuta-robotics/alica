...
ControlledProcessManager::ControlledProcessManager(
        string processManagerName, const supplementary::AgentID* processManagerId, QBoxLayout* parentLayout)
...
void ControlledProcessManager::updateGUI(chrono::system_clock::time_point now) {
    for (auto robotMapIter = this->controlledRobotsMap.begin(); robotMapIter != this->controlledRobotsMap.end();) {
        if ((now - robotMapIter->second->timeLastMsgReceived) >
                this->msgTimeOut) {  // time is over, erase controlled robot
            std::cout << "ControlledPM: The robot " << robotMapIter->second->name
                      << " (ID: " << robotMapIter->second->agentID << ") on process manager " << this->name
                      << " (ID: " << this->id << ") seems to be dead!" << std::endl;
