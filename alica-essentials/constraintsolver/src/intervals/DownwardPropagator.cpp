...
int DownwardPropagator::visit(shared_ptr<ConstPower> intPower) {
...
#endif
    if (intPower->max == numeric_limits<double>::infinity() || intPower->min == -numeric_limits<double>::infinity())
        return false;
    ...
    if (isRational) {
        if (updateInterval(intPower->base, std::max(0.0, std::min(a, b)), std::max(a, std::max(-a, std::max(b, -b))))) {
...
void DownwardPropagator::outputChange(shared_ptr<Term> t, double oldmin, double oldmax) {
    ...
    if (dynamic_pointer_cast<Variable>(t) != 0)
        cout << "DW shrinking [" << oldmin << ".." << oldmax << "] to [" << t->min << ".." << t->max << "] by "
