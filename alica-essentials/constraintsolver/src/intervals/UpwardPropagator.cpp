...
int UpwardPropagator::visit(shared_ptr<Max> max) {
    if (updateInterval(max, std::min(max->left->min, max->right->min), std::max(max->left->max, max->right->max))) {
        addChanged(max);
        return true;
    }
    return false;
...
int UpwardPropagator::visit(shared_ptr<Min> min) {
    if (updateInterval(min, std::min(min->left->min, min->right->min), std::max(min->left->max, min->right->max))) {
        addChanged(min);
        return true;
    }
    return false;
...
void UpwardPropagator::outputChange(shared_ptr<Term> t, double oldmin, double oldmax) {
    ...
    if (dynamic_pointer_cast<Variable>(t) != 0)
        cout << "UW shrinking [" << oldmin << ".." << oldmax << "] to [" << t->min << ".." << t->max << "] by "
             << (oldwidth - newwidth) << " (" << ((oldwidth - newwidth) / oldwidth * 100) << "%)" << endl;
}
