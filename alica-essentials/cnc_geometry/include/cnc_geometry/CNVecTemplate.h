...
public:
    ...
    double angleZTo(T point) const { return atan2(point->y - this->y, point->x - this->x); }
    ...
    // Self
    T operator+(const std::shared_ptr<T>& right) const { return T(this->x + right->x, this->y + right->y, this->z + right->z); }
