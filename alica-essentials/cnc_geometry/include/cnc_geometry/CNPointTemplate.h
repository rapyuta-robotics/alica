...
class CNPointTemplate : public geometry_msgs::Point
{
  public:
    ...
    double angleZToPoint(T& point) const { return atan2(point.y - this->y, point.x - this->x); }
    ...
    // Self
    T operator+(const CNPointTemplate<T>& right) const { return T(this->x + right.x, this->y + right.y, this->z + right.z); }

    T operator-(const CNPointTemplate<T>& right) const { return T(this->x - right.x, this->y - right.y, this->z - right.z); }

    // Scalar
    T operator/(const double& right) const { return T(this->x / right, this->y / right, this->z / right); }
