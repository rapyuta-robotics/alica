...
public:
    ...
    AgentIDConstPtr(const AgentID* id)
            : _ptr(id)
    {
    }
    const AgentID& operator*() const { return *_ptr; }
    const AgentID* operator->() const { return _ptr; }
    ...
    const AgentID* get() const { return _ptr; }
...
struct AgentIDConstPtrHash
{
    std::size_t operator()(const AgentIDConstPtr obj) const { return obj.hash(); }
};
}
...
{
template <>
struct hash<essentials::AgentIDConstPtr>
{
    std::size_t operator()(const essentials::AgentIDConstPtr id) const noexcept { return id.hash(); }
};
}
