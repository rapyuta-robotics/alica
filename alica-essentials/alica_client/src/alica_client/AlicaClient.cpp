...
void AlicaClient::initPlugin(qt_gui_cpp::PluginContext& context) {
    ...
    if (context.serialNumber() > 1) {
        alicaWidget->qframe->setWindowTitle(
                alicaWidget->qframe->windowTitle() + " (" + QString::number(context.serialNumber()) + ")");
    }
    ...
    aliceClientSubscriber = rosNode->subscribe(
            "/AlicaEngine/AlicaEngineInfo", 10, &alica::AlicaWidget::handleAlicaEngineInfo, this->alicaWidget);
}
...
void AlicaClient::restoreSettings(
