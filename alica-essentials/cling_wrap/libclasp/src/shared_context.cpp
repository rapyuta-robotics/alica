...
bool SharedContext::unfreezeStep() {
	for (uint32 i = 0; i != solvers_.size(); ++i) {
		...
		if (!isSentinel(step_) && s.simplify()) {
			LitVec::size_type i = s.assign_.trail.size();
			while (i--) {
				Var v = s.assign_.trail[i].var();
				if (v == step_.var()) {
					s.assign_.trail[i]     = s.assign_.trail.back();
					s.assign_.trail.back() = negLit(v);
					s.assign_.undoLast();
					break;
				}
			}
...
bool SharedContext::endInit(bool attachAll) {
	...
	initStats(*master());
	SatPrePtr temp(satPrepro.release());
	...
	if (concurrency() == 1 || master()->dbIdx_ == 0) {
		...
		if (rem) {
			for (SolverVec::size_type s = 1; s != solvers_.size(); ++s) {
				...
				else if (x.dbIdx_ != 0)         { x.dbIdx_ -= std::count_if(db.begin(), db.begin()+x.dbIdx_, IsNull()); }
			}
