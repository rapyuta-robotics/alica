...
public class DragDropEditPolicy extends GraphicalEditPolicy  {
	...
	public Command getOrphanChildrenCommand(GroupRequest request) {
		List<AbstractPlanStateEditPart> parts = request.getEditParts();
		CompoundCommand result = new CompoundCommand("ORPHAN_CHILDREN");
		final PMLTransactionalEditingDomain editingDomain =  (PMLTransactionalEditingDomain)TransactionUtil.getEditingDomain(getHost().getModel());
		getHost().showTargetFeedback(request);

		for (int i = 0; i < parts.size(); i++) {
			result.append(RemoveCommand.create(editingDomain, getHost().getModel(), AlicaPackage.eINSTANCE.getState_Plans(), (parts.get(i)).getModel()));
		}
		return new CommandWrap4EMF(result);
	}
...
//		}
		...
		if(getHost().getModel() instanceof EObject){
			...
			if (getHost() instanceof EntryPointEditPart) {
