 ...
 * </ul>
 * Each ExpandItem is draggable, the Transfer is a TemplateTransfer with it's
 * template set to the dragged IFile.
 * 
 * @author Zenobios
 * 
 */
public class RepositoryView extends ViewPart implements
		ICommandStackTabbedPropertySheetPageContributor {
	...
	private class TaskrepositoryLabelProvider extends LabelProvider {
		...
		public Image getImage(Object element) {
			Image img = null;
			if (element instanceof Task) {
				img = PlanDesignerActivator.getDefault().getImageRegistry()
						.get(PlanDesignerConstants.ICON_TASK_16);
			} else
	...
	private class TaskrepositoryContentProvider implements
		...
		public Object[] getElements(Object inputElement) {
			...
			if (inputElement instanceof IWorkspaceRoot) {
				...
				if (taskrepositoryFile.exists()) {
					...
					// to be correct
					TaskRepository repository = CommonUtils.getTaskRepository(
	...
	private class BehaviourTreeLabelProvider implements ILabelProvider {
		...
		public Image getImage(Object element) {
			if (element instanceof Behaviour)
				return PlanDesignerActivator.getDefault().getImageRegistry()
						.get(PlanDesignerConstants.ICON_BEHAVIOUR_16);
			else if (element instanceof BehaviourConfiguration)
				return PlanDesignerActivator
						.getDefault()
						.getImageRegistry()
						.get(PlanDesignerConstants.ICON_BEHAVIOUR_CONFIGURATION_16);
			else
				return PlanDesignerActivator.getDefault().getImageRegistry()
		...
		public String getText(Object element) {
			...
			if (element instanceof PlanElement) {
				return (String) PlanEditorUtils.safeLoadFeature(
						getEditingDomain(), (PlanElement) element,
	...
	private class BehaviourTreeContentProvider implements ITreeContentProvider {
		...
		public Object[] getChildren(final Object parentElement) {
			...
			if (parentElement instanceof Behaviour) {
				Object[] result = null;
				try {
					result = (Object[]) getEditingDomain().runExclusive(
							new RunnableWithResult.Impl<Object[]>() {
								public void run() {
									setResult(((Behaviour) parentElement)
		...
		public Object getParent(Object element) {
			if (element instanceof BehaviourConfiguration)
				return PlanEditorUtils.safeLoadFeature(getEditingDomain(),
						(BehaviourConfiguration) element,
						AlicaPackage.eINSTANCE
								.getBehaviourConfiguration_Behaviour());
			else
				return null;
		}
		...
		public boolean hasChildren(Object element) {
			if (element instanceof Behaviour
					&& hasConfigurations((Behaviour) element))
				return true;
			else
				return false;
		}
		...
		public Object[] getElements(Object inputElement) {
			...
			for (Behaviour behaviour : behaviours) {
				...
				for (BehaviourConfiguration config : behaviour
						.getConfigurations()) {
		...
		private boolean hasConfigurations(final Behaviour beh) {
			...
			try {
				result = (Boolean) getEditingDomain().runExclusive(
	...
	private class ViewContentProvider implements IStructuredContentProvider {
		...
		public Object[] getElements(Object parent) {
			final Set<IFile> resources = PlanEditorUtils
					.collectAllFilesWithExtension(fileExtension);
			...
			for (int i = 0; i < loaded.size(); i++) {
				if (!(loaded.get(i).getContents().isEmpty())) {
					loadedElements[i] = loaded.get(i).getContents().get(0);
				} else {
					System.out
							.println("RepoView: The following file is empty: "
									+ loaded.get(i).getURI());
				}
			}
	...
	private class ViewLabelProvider extends LabelProvider {
		...
		public String getText(Object obj) {
			if (obj instanceof PlanElement) {
				return (String) PlanEditorUtils.safeLoadFeature(
						getEditingDomain(), (PlanElement) obj,
						AlicaPackage.eINSTANCE.getPlanElement_Name());
			}
			return obj.toString();
		}
		...
		public Image getImage(Object obj) {
			ImageRegistry reg = PlanDesignerActivator.getDefault()
					.getImageRegistry();
	...
	public void createPartControl(Composite parent) {
		...
		tabItem.setImage(PlanDesignerActivator.getDefault().getImageRegistry()
				.get(PlanDesignerConstants.ICON_PLAN_16));

		// Create the viewer for the plans
		plansViewer = new TableViewer(folder, SWT.BORDER | SWT.MULTI
			...
			public void doubleClick(DoubleClickEvent event) {
				...
				if (selection instanceof EObject) {
					try {
						...
						IDE.openEditor(RepositoryView.this.getSite().getPage(),
								WorkspaceSynchronizer.getFile(element
										.eResource()));
						refreshView();
		...
		tabItem.setImage(PlanDesignerActivator.getDefault().getImageRegistry()
				.get(PlanDesignerConstants.ICON_PLANTYPE_16));

		// Create the viewer for the plantypes
		planTypesViewer = new TableViewer(folder, SWT.BORDER | SWT.MULTI
				| SWT.H_SCROLL | SWT.V_SCROLL);
		...
		planTypesViewer.addDoubleClickListener(new IDoubleClickListener() {
			public void doubleClick(DoubleClickEvent event) {
				...
				if (selection instanceof EObject) {
					...
					// Create a plantype configuration wizard and initialize it
					// with the plantype
					PMLPlanTypeConfigurationWizard wiz = new PMLPlanTypeConfigurationWizard(
							type);

					WizardDialog dialog = new WizardDialog(planTypesViewer
							.getControl().getShell(), wiz);
		...
		behaviourViewer = new TreeViewer(folder, SWT.BORDER | SWT.MULTI
				| SWT.H_SCROLL | SWT.V_SCROLL);
		...
		// Create the viewer for the behaviours
		taskrepositoryViewer = new TableViewer(folder, SWT.BORDER | SWT.MULTI
				| SWT.H_SCROLL | SWT.V_SCROLL);
		taskrepositoryViewer
				.setContentProvider(new TaskrepositoryContentProvider());
		taskrepositoryViewer
		...
		tabItem.setImage(PlanDesignerActivator.getDefault().getImageRegistry()
				.get(PlanDesignerConstants.ICON_PLANNING_PROBLEM_16));

		// Create the viewer for the plans
		planningProblemsViewer = new TableViewer(folder, SWT.BORDER | SWT.MULTI
				| SWT.H_SCROLL | SWT.V_SCROLL);
		planningProblemsViewer
				.setContentProvider(new ViewContentProvider("pp"));
				...
				.addDoubleClickListener(new IDoubleClickListener() {
					public void doubleClick(DoubleClickEvent event) {
						...
						if (selection instanceof EObject) {
							...
							// Create a plantype configuration wizard and
							// initialize it with the plantype
							PMLPlanningProblemConfigurationWizard wiz = new PMLPlanningProblemConfigurationWizard(
									pp);

							WizardDialog dialog = new WizardDialog(
									planningProblemsViewer.getControl()
											.getShell(), wiz);
		...
		planningProblemsViewer.getControl().addFocusListener(
				new FocusAdapter() {
					@Override
					public void focusGained(FocusEvent e) {
		...
		SelectionProviderMediator med = new SelectionProviderMediator(viewers,
				null);
	...
	private void makeActions() {
		deleteBehaviourConfigAction = new DeleteBehaviourConfigurationAction(
				behaviourViewer);
		deleteTaskAction = new DeleteTaskFromRepositoryAction(
				taskrepositoryViewer);
		deletePlan = new DeletePlanFromRepositoryAction(plansViewer);
		deletePlantypes = new DeletePlanTypesFromRepositryAction(
				planTypesViewer);
	}
	...
	private void hookContextMenu() {
		...
		getSite().registerContextMenu(
				PlanDesignerConstants.PML_REPOSITORY_ID
						+ ".repositoryContextMenu", menuMgr,
	...
	public EMF2GEFCommandStack getEMFCommandStack() {
		if (this.commandStack == null) {
			this.commandStack = (EMF2GEFCommandStack) getEditingDomain()
					.getCommandStack();
		}
		...
		if (behaviourNotificationAdapter == null) {
			behaviourNotificationAdapter = new MultiObjectNotificationAdapter() {
				...
				public void doNotify(Notification n) {
					// Since we only display PlanElement names in our view it is
					// sufficient
					// to react to this feature
					if (n.getFeature().equals(
							AlicaPackage.eINSTANCE.getPlanElement_Name()))
						refreshView();
				}
		...
		if (planningProblemNotificationAdapter == null) {
			planningProblemNotificationAdapter = new MultiObjectNotificationAdapter() {
				...
				public void doNotify(Notification n) {
					...
					// to react to this feature
					if (n.getFeature().equals(
							AlicaPackage.eINSTANCE.getPlanningProblem()
									.getName()))
						refreshView();
				}
	...
	public PMLTransactionalEditingDomain getEditingDomain() {
		if (editingDomain == null) {
			...
			editingDomain = (PMLTransactionalEditingDomain) TransactionalEditingDomain.Registry.INSTANCE
