...
public class ParametrisationSection extends PMLPropertySection {
			...
			try {
				result = (Object[])getEditingDomain().runExclusive(new RunnableWithResult.Impl<Object[]>(){
					public void run() {
						...
						for(AbstractPlan subp : s.getPlans()) {
	...
	private class ParametrisationEditingSupport extends EditingSupport{
		...
		protected boolean canEdit(Object element) {
			return ParametrisationSection.this.isEditable();
		}

		@Override
		protected CellEditor getCellEditor(Object element) {
			choices = ((State)getViewer().getInput()).getInPlan().getVars();
			...
			cmp.append(new AbstractCommand(){
				...
				RecordingCommand cmd = new RecordingCommand(getEditingDomain()){
					...
					protected void doExecute() {
						...
						Parametrisation p =(Parametrisation)element;
						EList<Parametrisation> pl = ((State)getViewer().getInput()).getParametrisation();
						if (index==0) {
							p.setVar(null);
							pl.remove(p);
						} else {
							EList<Variable> cv = ((State)getViewer().getInput()).getInPlan().getVars();
				...
				public void redo() {
					cmd.redo();
					if(!ParametrisationSection.this.getTViewer().getControl().isDisposed())
						updateView(null);
				}
				@Override
				public void undo() {
	...
	public void createControls(Composite parent,
		...
		tViewer.getTable().setSortColumn(column.getColumn());
		...
		tViewer.getTable().setSortDirection(SWT.UP);
