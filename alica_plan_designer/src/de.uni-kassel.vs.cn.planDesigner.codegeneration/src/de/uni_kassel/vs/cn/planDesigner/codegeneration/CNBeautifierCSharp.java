...
public class CNBeautifierCSharp implements PostProcessor {
	private static final String DEFAULT_CDT_OPTIONS = "ConfigCSHARP.xml";
	...
	private InputStream getConfig(final String filename) throws IOException {
		URL url = ResourceLoaderFactory.createResourceLoader().getResource(
				filename);
		if (url == null) {
			url = CNBeautifierCSharp.class.getResource(filename);
		}
		if (url == null) {
