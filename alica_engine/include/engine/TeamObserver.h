...
public:
    ...
    const std::map<uint64_t, std::unique_ptr<SimplePlanTree>>& getTeamPlanTrees() const { return _simplePlanTrees; }
...
private:
    ...
    std::unique_ptr<SimplePlanTree> sptFromMessage(uint64_t agent, const IdGrp& ids, AlicaTime time) const;
    ...
    std::map<uint64_t, std::unique_ptr<SimplePlanTree>> _simplePlanTrees;
