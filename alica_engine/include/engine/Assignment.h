...
public:
    AgentStatePairs() {}
    bool hasAgent(const essentials::AgentIDConstPtr id) const;
    const State* getStateOfAgent(const essentials::AgentIDConstPtr id) const;
    void setStateOfAgent(const essentials::AgentIDConstPtr id, const State* s);
    ...
    void emplace_back(essentials::AgentIDConstPtr id, const State* s) { _data.emplace_back(id, s); }

    void removeAt(int idx) { _data.erase(_data.begin() + idx); }
    void remove(essentials::AgentIDConstPtr agent)
    {
...
public:
    ...
    bool hasAgent(essentials::AgentIDConstPtr id) const;
    int size() const
    {
    ...
    const EntryPoint* getEntryPointOfAgent(essentials::AgentIDConstPtr id) const;
    const State* getStateOfAgent(essentials::AgentIDConstPtr id) const;
    ...
    bool updateAgent(essentials::AgentIDConstPtr agent, const EntryPoint* e);
    bool updateAgent(essentials::AgentIDConstPtr agent, const EntryPoint* e, const State* s);
    void addAgent(essentials::AgentIDConstPtr agent, const EntryPoint* e, const State* s) { _assignmentData[e->getIndex()].emplace_back(agent, s); }
    ...
    void setState(essentials::AgentIDConstPtr agent, const State* s, const EntryPoint* hint) { _assignmentData[hint->getIndex()].setStateOfAgent(agent, s); }
    ...
    void removeAgentFrom(essentials::AgentIDConstPtr agent, const EntryPoint* ep) { _assignmentData[ep->getIndex()].remove(agent); }
    void removeAgent(essentials::AgentIDConstPtr agent);
    ...
    for (int i = 0; i < static_cast<int>(_assignmentData.size()); ++i) {
        ...
        if (isTargetEp) {
            ...
            for (ForwardIterator agent_it = begin; agent_it != end; ++agent_it) {
                essentials::AgentIDConstPtr id = *agent_it;
...
class AssignmentIterator : public std::iterator<std::forward_iterator_tag, essentials::AgentIDConstPtr>
{
public:
    ...
    essentials::AgentIDConstPtr operator*() const { return _agents->getRaw()[_idx].first; }
...
class AllAgentsIterator : public std::iterator<std::forward_iterator_tag, essentials::AgentIDConstPtr>
{
public:
    ...
    essentials::AgentIDConstPtr operator*() const { return _assignment->getAgentStates(_epIdx).getRaw()[_agentIdx].first; }
...
class AgentsInStateIterator : public std::iterator<std::forward_iterator_tag, essentials::AgentIDConstPtr>
{
public:
...
class AssignmentSuccessIterator : public std::iterator<std::forward_iterator_tag, essentials::AgentIDConstPtr>
{
public:
