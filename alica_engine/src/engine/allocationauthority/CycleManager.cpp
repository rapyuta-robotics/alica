...
void CycleManager::update()
{
    ...
    if (_state == CycleState::observing) {
        if (detectAllocationCycle()) {
            Logging::logInfo(LOGNAME) << "Cycle Detected!";
            ...
            Logging::logDebug(LOGNAME) << "Assuming Authority for " << plan->getAuthorityTimeInterval().inSeconds() << "sec!";
        ...
        if (_state == CycleState::overriding && _overrideTimestamp + plan->getAuthorityTimeInterval() < _clock.now()) {
            Logging::logDebug(LOGNAME) << "Resume Observing!";
            _state = CycleState::observing;
...
void CycleManager::setNewAllocDiff(const Assignment& oldAss, const Assignment& newAss, AllocationDifference::Reason reas)
{
    ...
    Logging::logDebug(LOGNAME) << "SetNewAllDiff(b): " << aldif;
}
...
void CycleManager::handleAuthorityInfo(const AllocationAuthorityInfo& aai)
{
    ...
    if (rid < _myID) {
        Logging::logInfo(LOGNAME) << "Rcv: Rejecting Authority!";
        if (_state != CycleState::overriding) {
            Logging::logDebug(LOGNAME) << "Overriding assignment of " << _runningPlan->getActivePlan()->getName();
        ...
        Logging::logDebug(LOGNAME) << "Assignment overridden in " << _runningPlan->getActivePlan()->getName();
...
{
    Logging::logDebug(LOGNAME) << "Setting authorative assignment for plan " << _runningPlan->getActivePlan()->getName();
