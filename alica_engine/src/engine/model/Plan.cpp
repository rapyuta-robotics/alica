...
void Plan::computeDynamicEntryPoints() const
{
    ...
    for (auto ep : _entryPoints) {
        ...
        for (auto id : dynamicIds) {
            std::pair<int64_t, int64_t> p{ep->getId(), id};
            auto it = _entryPointCache.find(p);
            if (it != _entryPointCache.end()) {
                _allEntryPoints.push_back(it->second);
            } else {
                EntryPoint* nep = new EntryPoint(*ep);
                nep->setDynamicId(id);
                _entryPointCache.insert({p, nep});
                _allEntryPoints.push_back(nep);
            }
        }
    }
    constexpr std::size_t MAX_CACHE_SIZE = 1000;
    while (_entryPointCache.size() > MAX_CACHE_SIZE) {
        _entryPointCache.erase(_entryPointCache.begin());
    }
}
