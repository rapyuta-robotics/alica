...
ForallAgents::Result ForallAgents::TryAddId(
        AgentIDPtr id, std::vector<AgentVariables>& io_agentVarsInScope, const TeamManager* tm) const {
    std::vector<AgentVariables>::iterator it = std::find_if(io_agentVarsInScope.begin(), io_agentVarsInScope.end(),
...
bool ForallAgents::isAgentInScope(AgentIDPtr id, std::shared_ptr<RunningPlan>& rp) const {
    switch (getScopeType()) {
        case PLANSCOPE:
            return rp->getPlan() == getScopedPlan() &&
                   std::find(rp->getRobotsAvail().begin(), rp->getRobotsAvail().end(), id) !=
...
bool ForallAgents::addDomainVariables(
        std::shared_ptr<RunningPlan>& p, std::vector<AgentVariables>& io_agentVarsInScope) const {
