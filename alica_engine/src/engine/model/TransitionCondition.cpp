...
bool TransitionCondition::evaluate(const RunningPlan* rp, const Blackboard* worldModels, const KeyMapping* keyMapping)
{
    if (rp->isBehaviour()) {
        return false;
    }
    ...
    return _evalCallback(_blackboard.get(), rp, worldModels);
}
