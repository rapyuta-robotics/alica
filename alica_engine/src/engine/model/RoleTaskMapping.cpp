...
std::string RoleTaskMapping::toString() const
{
    ...
    for (std::unordered_map<int64_t, double>::const_iterator iterator = _taskPriorities.begin(); iterator != _taskPriorities.end(); ++iterator) {
...
void RoleTaskMapping::setTaskPriorities(const std::unordered_map<int64_t, double>& taskPriorities)
{
