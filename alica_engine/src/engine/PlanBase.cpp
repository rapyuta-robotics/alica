        ...
        while (_running)
        {
            ...
            if (_ae->getStepEngine())
            {
...
#endif
                {
                    ...
                    _isWaiting = true;
                    AlicaEngine* ae = _ae;
                    _stepModeCV.wait(lckStep, [ae]
                    {
                        return ae->getStepCalled();
                    });
            ...
            if (_sendStatusMessages && _lastSentStatusTime + _sendStatusInterval < _alicaClock->now())
            {
                ...
                if (_deepestNode != nullptr)
                {
                    ...
                    if (_deepestNode->getActiveState() != nullptr)
                    {
                        ...
                        std::copy(_deepestNode->getAssignment()->getRobotStateMapping()->getRobotsInState(
                                _deepestNode->getActiveState()).begin(),
                                _deepestNode->getAssignment()->getRobotStateMapping()->getRobotsInState(
                                        _deepestNode->getActiveState()).end(),
