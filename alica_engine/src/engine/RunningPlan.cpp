    ...
    if (_activeTriple.state != s) {
        ...
        if (s != nullptr) {
            if (s->isFailureState()) {
                ...
                _ae->editTeamManager().setSuccess(mid, _parent ? _parent->_contextHash : contextHash(0), _activeTriple.entryPoint);
            }
        }
    }
}
...
bool RunningPlan::recursiveUpdateAssignment(const std::vector<const SimplePlanTree*>& spts, AgentGrp& availableAgents, const AgentGrp& noUpdates, AlicaTime now)
{
    ...
    _ae->editTeamObserver().updateSuccessCollection(
            _parent ? _parent->getContextHash() : contextHash(0), static_cast<const Plan*>(getActivePlan()), _assignment.editSuccessData());
