...
{
    _syncStartTime = _alicaClock->now();
    for (const Transition* t : sync->getInSync()) {
        _connectedTransitions.push_back(t->getId());
    }
}

SynchronisationProcess::SynchronisationProcess(std::shared_ptr<AlicaClock> clock, AgentId myID, const Synchronisation* sync, SyncModule* sm)
        : _myID(myID)
        , _synchronisation(sync)
        , _syncModule(sm)
        , _readyForSync(false)
...
bool SynchronisationProcess::integrateSyncTalk(std::shared_ptr<SyncTalk> talk, uint64_t curTick)
{
    ...
    for (const SyncData& sd : talk->syncData) {
        std::lock_guard<mutex> lock(_syncMutex);
        {
            ...
            if (_readyForSync) {
                if (allSyncReady()) {
                    ALICA_DEBUG_MSG("[SP (" << _myID << ")]: Synchronisation successful (IntTalk) - elapsed time: " << (_alicaClock->now() - _syncStartTime));
                    // notify sync module
...
void SynchronisationProcess::integrateSyncReady(shared_ptr<SyncReady> ready)
{
    ...
    if (_readyForSync) {
        if (allSyncReady()) {
            // notify _syncModul
            ALICA_DEBUG_MSG("[SP (" << _myID << ")]: Synchronisation successful (IntReady) - elapsed time: " << (_alicaClock->now() - _syncStartTime));
            _syncModule->synchronisationDone(_synchronisation);
            _synchronisationDone = true;
        }
    }
