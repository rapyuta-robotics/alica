...
SyncModule::SyncModule(const AlicaEngine* ae)
        ...
        , _running(false)
        , _ticks(0)
        , _teamManager(ae->getTeamManager())
        , _planRepository(ae->getPlanRepository())
        , _maySendMessages(ae->maySendMessages())
        , _communicator(nullptr) // tmp
        , _alicaClock(ae->getAlicaClockPtr())
{
}

SyncModule::SyncModule(const TeamManager& teamManager, const PlanRepository& planRepository, const bool& maySendMessages,
...
void SyncModule::setSynchronisation(const Transition* trans, bool holds)
{
    ...
    if (i != _synchProcessMapping.end()) {
        ...
        SynchronisationProcess* synchProcess = new SynchronisationProcess(_alicaClock, _myId, trans->getSynchronisation(), this);
    ...
    for (const SyncData& sd : st->syncData) {
        ...
        bool doAck = true;
        {
            ...
            if (i != _synchProcessMapping.end()) {
                ...
                SynchronisationProcess* syncProc = new SynchronisationProcess(_alicaClock, _myId, synchronisation, this);
...
void SyncModule::setCommunicator(std::shared_ptr<IAlicaCommunication> communicator)
{
    _communicator = communicator;
}

void SyncModule::setAlicaClock(std::shared_ptr<AlicaClock> clock)
{
    _alicaClock = clock;
}
