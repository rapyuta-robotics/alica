...
bool PartialAssignment::expand(std::vector<PartialAssignment*>& o_container, PartialAssignmentPool& pool, const Assignment* old, const Blackboard* worldModels)
{
    ...
    for (int i = 0; i < numChildren; ++i) {
        if (_cardinalities[i].getMax() > 0) {
            ...
            *newPa = *this;
            ...
            newPa->evaluate(old, worldModels);
