...
RuntimeBehaviourFactory::RuntimeBehaviourFactory(
        std::unique_ptr<IBehaviourCreator>&& bc, IAlicaWorldModel* wm, const IAlicaTraceFactory* tf, AlicaEngine* engine)
...
{
    BehaviourContext ctx{_wm, behaviourModel->getName(), behaviourModel, _tf};
    ...
    // TODO Cleanup: get rid of this later, behaviour only needs teamManager and not entire engine
