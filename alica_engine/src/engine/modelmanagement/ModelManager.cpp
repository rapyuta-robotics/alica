...
Plan* ModelManager::loadPlanTree(const std::string& masterPlanName, std::optional<std::string> placeholderMapping)
{
    ...
    if (placeholderMapping.has_value()) {
        /*
         * Although placeholderMapping is not technically an AlicaElement because it is not generated by the designer, the implementation is simplified if it is
         * treated as such because we can then re-use the same methods of 1. reading elements (placeholders & abstract plans) referenced in the mapping file &
         * 2. attaching references after the elements are read, through the AlicaElement & Factory class hierarchies
         */
        YAML::Node node;
        node["id"] = -1;
        node["name"] = "Mapping";
        node["comment"] = "Mapping auto generated";
        node["mapping"] = YAML::Load(placeholderMapping.value());
        PlaceholderMappingFactory::create(node);
    }
