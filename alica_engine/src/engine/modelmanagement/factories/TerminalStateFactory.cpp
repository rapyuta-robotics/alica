...
TerminalState* TerminalStateFactory::create(const YAML::Node& terminalStateNode, AbstractPlan* plan)
{
    ...
    terminalState->setInPlan((Plan*) Factory::getElement(Factory::getReferencedId(terminalStateNode[alica::Strings::parentPlan])));
    if (Factory::isValid(terminalStateNode[alica::Strings::postCondition])) {
        terminalState->setPostCondition(PostConditionFactory::create(terminalStateNode[alica::Strings::postCondition], plan));
    }
