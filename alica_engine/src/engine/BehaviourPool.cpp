...
bool BehaviourPool::init(IBehaviourCreator& bc)
{
    ...
    for (const ConfAbstractPlanWrapper* wrapper : wrappers) {
        if (const auto* behaviour = dynamic_cast<const Behaviour*>(wrapper->getAbstractPlan())) {
            if (getBasicBehaviour(behaviour)) {
                // A BasicBehaviour representing this combination of Behaviour and Configuration was created already!
                continue;
            }

            // Create a new BasicBehaviour object. Note that a single BasicBehaviour object will be
            // created for all configurations of the behaviour
            ...
            basicBeh->init();
            _availableBehaviours[behaviour->getId()] = basicBeh;
        }
    }
    return true;
}
...
void BehaviourPool::startBehaviour(RunningPlan& rp)
{
    if (const auto* beh = dynamic_cast<const Behaviour*>(rp.getActivePlan())) {
        if (auto& bb = getBasicBehaviour(beh)) {
...
{
    if (const auto* beh = dynamic_cast<const Behaviour*>(rp.getActivePlan())) {
        auto& bb = getBasicBehaviour(beh);
        return bb && bb->isRunningInContext(&rp);
    }
    return false;
}

const std::shared_ptr<BasicBehaviour> BehaviourPool::getBasicBehaviour(const Behaviour* behaviour) const
{
    auto it = _availableBehaviours.find(behaviour->getId());
    if (it != _availableBehaviours.end()) {
        return it->second;
    }
    return nullptr;
}
