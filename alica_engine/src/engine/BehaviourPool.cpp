...
bool BehaviourPool::init(IBehaviourCreator& bc)
{
    ...
    for (const ConfAbstractPlanWrapper* wrapper : wrappers) {
        if (const auto* behaviour = dynamic_cast<const Behaviour*>(wrapper->getAbstractPlan())) {
            ...
            BehaviourContext ctx{_ae->getWorldModel(), behaviour->getName(), behaviour};
            auto basicBeh = bc.createBehaviour(behaviour->getId(), ctx);
            if (!basicBeh) {
            ...
            basicBeh->init();
            _availableBehaviours.insert(std::make_pair(wrapper, std::move(basicBeh)));
        }
    }
    return true;
}
